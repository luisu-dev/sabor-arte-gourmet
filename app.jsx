...
function WhatsAppFloat() {
  ...
  const [form, setForm] = useState({ nombre: "", whatsapp: "", correo: "", empresa: "", ciudad: "" });
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const msg =
      `Hola, me interesa conocer más sobre Sabor & Arte.\n\n` +
      `Nombre: ${form.nombre}\n` +
      `WhatsApp: ${form.whatsapp}\n` +
      `Correo: ${form.correo}\n` +
      `Empresa: ${form.empresa}\n` +
      `Ciudad: ${form.ciudad}`;
    window.open(`https://wa.me/529841872094?text=${encodeURIComponent(msg)}`, "_blank");
    setOpen(false);
    setForm({ nombre: "", whatsapp: "", correo: "", empresa: "", ciudad: "" });
  }

  return (
    <>
      {open && (
        <div className="wa-overlay" onClick={() => setOpen(false)}>
          <div className="wa-modal" onClick={(e) => e.stopPropagation()}>
            <button className="wa-modal-close" onClick={() => setOpen(false)} aria-label="Cerrar">&times;</button>
            ...
            <p className="wa-modal-sub">Déjanos tus datos y te contactamos de inmediato.</p>
            <form className="wa-form" onSubmit={handleSubmit}>
              <input name="nombre"   type="text"  placeholder="Nombre completo"        value={form.nombre}   onChange={handleChange} required />
              <input name="whatsapp" type="tel"   placeholder="Número WhatsApp"         value={form.whatsapp} onChange={handleChange} required />
              <input name="correo"   type="email" placeholder="Correo electrónico"      value={form.correo}   onChange={handleChange} required />
              <input name="empresa"  type="text"  placeholder="Empresa / Negocio"       value={form.empresa}  onChange={handleChange} required />
              <input name="ciudad"   type="text"  placeholder="Ciudad"                  value={form.ciudad}   onChange={handleChange} required />
              <button type="submit" className="wa-form-submit">
      ...
      <button className="wa-float-btn" onClick={() => setOpen(true)} aria-label="WhatsApp">
        ...
        <div className="container footer-grid">
          <div>
            ...
            <div className="footer-social">
              ...
              <a className="social-btn facebook" href="https://web.facebook.com/profile.php?id=61586470804121&locale=es_LA" target="_blank" rel="noreferrer" aria-label="Facebook">
